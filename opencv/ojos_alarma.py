...
EAR_THRESHOLD = 0.2
CLOSED_DURATION_THRESHOLD = 2  # En segundos
...
playing_sound = False  # Verifica si el sonido está reproduciéndose

# Cargar archivo de sonido
wave_obj = sa.WaveObject.from_wave_file("alerta.wav")  #se cambia el nombre de sonido 
...
while True:
    ...
    for face in faces:
        ...
        if ear < EAR_THRESHOLD:
            ...
            # Detener el sonido si los ojos están abiertos
            if playing_sound:
                playing_sound = False
    ...
    if eyes_are_closed and eyes_closed_start_time:
        ...
        if elapsed_time >= CLOSED_DURATION_THRESHOLD and not eyes_closed_printed:
            ...
            if not playing_sound:  # Reproducir sonido si no se está reproduciendo
...
# Asegurarse de detener el sonido al salir
playing_sound = False
cap.release()
